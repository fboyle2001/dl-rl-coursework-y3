...
class SVGSACAgent(RLAgent):
    ...
    def __init__(self, env_name: str, device: Union[str, torch.device], video_every: Optional[int],
                    ...
                    multi_step_updates_n_seq: int = 4, multi_step_batch_size: int = 1024,
        ...
        # Alpha can get very close to 0 so for the same reason as in
        # the GaussianActor we optimise ln(alpha) instead for numerical stability
        self.log_alpha = torch.zeros(np.log(alpha_init), requires_grad=True, device=self.device)

        # World model
        self.dynamics_model = networks.GRUDynamicsModel(self._state_dim + self._action_dim, self._state_dim, horizon,
                                                        self.device, clip_grad=clip_grad, lr=world_model_lr)
    ...
    def _train_reward_model(self, states, actions, expected_rewards) -> None:
        ...
        loss = F.mse_loss(self.reward_model(joined), expected_rewards)
    ...
    def train_policy(self) -> None:
        # Need to fill the buffer before we can do anything
        if self.replay_buffer.count < self.single_step_batch_size:
            return

        if self.steps % self.dynamics_update_freq == 0:
            for _ in range(self.dynamics_update_repeat):
                trajectories = self.replay_buffer.sample_trajectories(self.multi_step_batch_size, self.multi_step_updates_n_seq)
                states, actions, rewards = None, None, None
                dynamics_loss = self.dynamics_model.train(trajectories[:, 0], trajectories[:, 1], trajectories[:, 2])

        n_updates = 1 if self.steps < self.warmup_steps else self.model_free_update_repeat

        for _ in range(n_updates):
            # Firstly update the Q functions (the critics)
            buffer_sample = self.replay_buffer.sample_buffer(self.single_step_batch_size)
            ...
            if self.steps % self.actor_update_freq == 0:
                ...
                if model_free_update:
                    ...
                    predicted_states, policy_actions, log_probs = self.dynamics_model.unroll_horizon(buffer_sample.states, self.actor)
                    all_states = torch.cat([predicted_states, buffer_sample.states.unsqueeze(0)], dim=0)
                ...
                self._train_reward_model(states, actions, rewards)
                self._train_termination_model(states, actions, terminals) # Does this need to be 1 - terminals?
    ...
    def run(self) -> None:
        while self._episodes < self._max_episodes:
            ...
            while not done: 
                ...
                store_done = done if self._steps < self.env.spec._max_episode_steps else False # type: ignore

                self.replay_buffer.store_replay(state, action, reward, next_state, store_done)
                state = next_state 
